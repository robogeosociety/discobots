...
| "how should I get to work from Capitol Hill" / "transit near X" / "buses near here" / "what trains are close" | `./mapbot.py commute "Capitol Hill"` |
...
| "how should I get to work from Capitol Hill?" / "what transit is near X?" / "buses/trains near here" | `./mapbot.py commute "Capitol Hill"` (map + nearby stops with live departures) |
...
`commute` answers "how do I get to work from here?" style questions — it finds all bus and Link
stops within an 800m walk of a place (or the last-rendered location), fetches live departures for
each, and renders a **map + text summary**. Green pins = Link, orange = bus. Attach the PNG and
relay the stop-by-stop text. Use `--radius N` to widen/narrow the search area.
