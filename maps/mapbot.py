...
def do_commute(a):
    """Show nearby bus and train options with live departures — transit routing from a place."""
    import transit
    if a.place:
        g = resolve(a.place)
        lat, lon, label = g["lat"], g["lon"], short(g["name"])
    else:
        st = load_state()
        lat, lon = st["center"]["lat"], st["center"]["lon"]
        label = st.get("label", "current location")

    radius = a.radius or 800
    all_stops = transit.nearby_stops(lat, lon, radius)
    if not all_stops:
        sys.exit("no transit stops found within %dm of %s" % (radius, label))

    # Surface Link stops first (they're often not the geographically closest), then buses
    link_route_ids = set(transit.LINK_ROUTES.keys())
    link_stops = [s for s in all_stops if any(r in link_route_ids for r in s.get("routeIds", []))]
    bus_stops = [s for s in all_stops if s not in link_stops]
    stops = (link_stops[:2] + bus_stops)[:8]
    ...
    for i, stop in enumerate(stops):
        if i:
            time.sleep(0.3)
        try:
            deps, sits = transit.arrivals(stop["id"], minutes=45, limit=3)
        except Exception:
            deps, sits = [], []
        results.append((stop, deps, sits))

    # Map: blue anchor + color-coded stop pins (green/blue for Link, orange for bus)
    markers = [{"lon": lon, "lat": lat, "color": "2266ff", "size": "l"}]
    for stop, _, _ in results:
        is_link = any(r in link_route_ids for r in stop.get("routeIds", []))
        color = "38B030" if is_link else "e87722"
        markers.append({"lon": stop["lon"], "lat": stop["lat"], "color": color, "size": "s"})
    out = render(markers, "auto", DEFAULT_STYLE)
    save_state({"lon": lon, "lat": lat}, 15, DEFAULT_STYLE, markers, label)
    print(out)

    print("Transit near %s (%d stops within %dm):" % (label, len(stops), radius))
    for stop, deps, sits in results:
        d = haversine(lon, lat, stop["lon"], stop["lat"])
        print("  %s  (%.0fm)" % (stop["name"], d * 1000))
        if not deps:
            print("    no departures in the next 45 min")
        for dep in deps:
            when = "due" if dep["mins"] <= 0 else "%d min" % dep["mins"]
            live = "  live" if dep["live"] else ""
            print("    %-6s  %-28s  %s%s" % (
                dep["route"],
                (dep.get("headsign") or "")[:28],
                when, live,
            ))
        for sit in sits[:1]:
            msg = sit.get("summary") or sit.get("desc")
            if msg:
                print("    alert: %s" % msg[:100])
...
def main():
    ...
    cm = sub.add_parser("commute"); cm.add_argument("place", nargs="?")
    cm.add_argument("--radius", type=int, help="search radius in metres (default 800)")
    cm.set_defaults(fn=do_commute)
