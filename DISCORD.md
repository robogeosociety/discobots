...
| **`hooks/claude-session-summary/`** — #dev digests of every substantial Claude Code session; installed on Air + mini as a `SessionEnd` hook + daily launchd sweep (see its README) | `DISCORD_WEBHOOK_GITHUB` (the #dev webhook), copied to `~/.claude/hooks/dev-summary.env` per machine | `robogeosociety/discobots` |
