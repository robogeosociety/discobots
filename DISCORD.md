...
| --- | --- | --- | --- | --- |
| **tommybot** | mini: `~/dev/tommybot/.env` (internal-disk checkout) | `DISCORD_BOT_TOKEN`, `DISCORD_CHANNELS` | Main Obsidian-RAG bot (RAG over vault + InfluxDB). Hosted on the **mini** via launchd `com.tommybot.bot-mini` — live since 2026-07-01, tool-calls enabled (incl. `obsidian_*`, co-located with the wrapper). **Claude-backed** per the 2026-07-04 mini local-inference deprecation: answers via the Claude API (rebackend PR in flight in `robogeosociety/tommybot`); retrieval stays local through `vecserve` (:8899) + InfluxDB. **No local model on the mini** — the retired tier ran uniform-4bit Qwen3-**4B** (this row previously misstated it as 1.7B). The Air can run it too (`com.tommybot.bot`, its own `.env`, local 8B/14B tiers) but is currently dormant; only one host should hold a live gateway connection on a given token at a time. Also the **fallback token** for obsidian-automations. | `robogeosociety/tommybot` |
...
| **ModelBot** (`discord-models`) | `~/.claude/channels/discord-models/.env` | `DISCORD_BOT_TOKEN` (+ `access.json`) | Channel-session Claude Code bot for **#models**: read and swap the mini's single global base model by driving the `tommybot model list/use` CLI (tommybot#79) — `use <name> --save` then a `com.tommybot.serve-mini` restart. **Role ending:** the 2026-07-04 mini local-inference deprecation removes the base model this bot exists to swap; the #models channel persona is to be **repurposed or retired — flagged, not yet decided**. Ships no tool code — its workspace (`channels/models/`) is just the persona. Bot-per-purpose; pin a cheap `SESSION_MODEL` for mechanical tool-calling. | `robogeosociety/discobots` (persona under `channels/models/`) |
