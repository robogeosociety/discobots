...
| **ModelBot** (`discord-models`) | `~/.claude/channels/discord-models/.env` | `DISCORD_BOT_TOKEN` (+ `access.json`) | Channel-session Claude Code bot for **#models**: reads and hot-swaps the mini's single global base model by driving the `tommybot model list/use/current` CLI (tommybot#79). Ships no tool code — its workspace (`channels/models/`) is just the persona. Bot-per-purpose; pin a cheap `SESSION_MODEL` for mechanical tool-calling. | `tommyroar/discobots` (persona under `channels/models/`) |
